...
if __name__ == '__main__':
    ...
    if args.fit == 'discontinuity':     # always executed
    ...
    if args.fit == 'per_vertex':        # never executed
    ...
    elif args.fit == 'per_edge':        # never executed
    ...
    if args.png.suffix == '.npy':
        ...
        depth_translate = depth_data.min()
        depth_scale = depth_data.max() - depth_data.min()
        depth_data = (depth_data - depth_translate) / depth_scale
    ...
    # For Belhe Experiment only, the input SVG curves are only used to regularize the meshing. They are not discontinuities.
    # We keep a backup of it nontheless.
    mesh.e_discont_bk = deepcopy(mesh.e_discont)
    mesh.e_discont = np.array([], dtype=mesh.e_discont.dtype)

    # Visualize input mesh
    if True:
        ax = plot_mesh(mesh, discontinuity=True)
        plt.savefig(args.snapshot / 'mesh_after_triwild.svg')
        plt.close()

    # Dump triwild init mesh
    pickle_file_path = args.snapshot / 'mesh_init_triwild.pkl'
    with open(pickle_file_path, "wb") as pickle_file:
        pickle.dump(mesh, pickle_file)
    ...
    # Read image
    if not depth_flag:  # This block is never executed
        image = Image.open(args.png)
        image = image.convert('RGB')
        image_norm = torch.from_numpy(normalize_image(image)).to(device)
    else:               # This block is always executed
        image = Image.open(temp_png)
        image = image.convert('L')
        image_norm = torch.from_numpy(depth_data).to(device)

    # 1. Run DefGrid optimization (always skipped because of args.fit)
    if not args.dmesh and args.fit != 'discontinuity':      # Never executed
        mesh_simp = mesh
    ...
    if 'spp' in nerf2D_config and (args.fit != 'per_vertex') and \
            (args.fit != 'per_edge') and (args.fit != 'discontinuity'): # Never executed because of args.fit
        if args.sample:
    ...
    # Final output
    raw_spp = mlp_hybrid.for_sharpnet((0.00001,0.99999,0.00001,0.99999), resolution=2049, dim=1)
    raw_spp = raw_spp * depth_scale + depth_translate
    np.save(args.snapshot / 'output.npy', raw_spp)
